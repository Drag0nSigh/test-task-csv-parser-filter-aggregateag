...
def mock_field_types():
    """
    Создаёт словарь типов полей.

    Словарь типов полей для тестирования фильтрации, агрегации и сортировки.
    """
    return {
        'name': str,
        'brand': str,
...
@pytest.fixture
def mock_goods():
    """Создаёт список мок-объектов товаров для тестирования."""
    good1 = MagicMock()
    good1.__dict__ = {
        'name': 'iphone',
        'brand': 'apple',
...
def test_filter_goods(
        condition,
        expected_result,
        mock_goods,
        mock_field_types
):
    """Тест фильтрация товаров по заданным условиям."""
    ...
    assert ([item.__dict__ for item in result] ==
            [item.__dict__ for item in expected_mocks])
...
def test_calculate_aggregation(
    ...
    """Тест вычисление агрегации (min, max, avg) для числовых полей."""
...
def test_sort_goods(
    ...
    """Тест сортировки товаров по указанному полю и порядку."""
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            'brand>iphone',
            [[]],
            re.escape('Для строкового поля "brand" поддерживаются '
                      'только операторы = и !=')
        ),
        (
            'price>iphone',
            [[]],
            re.escape('Для числового поля "price" ожидается числовое значение,'
                      ' получено: iphone')
        ),
        (
            ' ; | ;',
...
def test_valid_filter_parse_condition(
    ...
    """
    Тест парсинг условий фильтрации.

    Тест с валидными и невалидными входными данными.
    """
    if expected_result == [[]]:
...
def test_valid_filter_compare(field, operator, value, result, mock_goods):
    """Тест сравнение одного товара с заданным условием фильтрации."""
